  ...
  return async function (dispatch) {
    try {
      const resul = await axios.get(
        `http://localhost:3001/products/filterBy?name=${name}`
      );
      return dispatch({
        type: SEARCH_PRODUCT,
        payload: resul.data,
...
export const getProductsFiltered = (filter) => {
  return async (dispatch) => {
    const { category, name, minPrice, maxPrice } = filter;
...
export function maxVisits(number) {
  return async function (dispatch) {
    try {
      const resul = await axios.get(
        `http://localhost:3001/products/max?visit=${number}`
      );
      return dispatch({
        type: MAX_VISITS,
        payload: resul.data,
...
export const statusFalse = (id) => {
  return async function () {
    const resp = await axios.put(
      "http://localhost:3001/products/status/false",
      id
    );
    console.log(resp);
    return resp;
...
export const statusTrue = (id) => {
  return async function () {
    const resp = await axios.put(
      "http://localhost:3001/products/status/true",
      id
    );
    return resp;
  };
...
export const visits = (id, visit) => {
  return async function () {
    const resp = await axios.put("http://localhost:3001/products/visits", {
      id,
      visit: visit,
    });
